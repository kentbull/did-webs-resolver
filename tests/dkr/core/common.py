...
def setup_habs():
    with (
        habbing.openHby(name='test', temp=True) as hby,
        habbing.openHby(name='wes', salt=signing.Salter(raw=b'wess-the-witness').qb64, temp=True) as wesHby,
        habbing.openHby(name='wis', salt=signing.Salter(raw=b'wiss-the-witness').qb64, temp=True) as wisHby,
        ...
        hab = hby.makeHab(name='cam', isith='1', nsith='1', icount=1, ncount=1, toad=1, wits=wits)
        ...
        msgs = bytearray()
        msgs.extend(wesHab.makeEndRole(eid=wesHab.pre, role=kering.Roles.controller, stamp=helping.nowIso8601()))
        ...
        msgs.extend(wisHab.makeEndRole(eid=wisHab.pre, role=kering.Roles.controller, stamp=helping.nowIso8601()))
        ...
        # Set up
        msgs.extend(hab.makeEndRole(eid=hab.pre, role=kering.Roles.controller, stamp=helping.nowIso8601()))
        ...
        msgs.extend(hab.makeEndRole(eid=agentHab.pre, role=kering.Roles.registrar, stamp=helping.nowIso8601()))

        msgs.extend(hab.makeEndRole(eid=agentHab.pre, role=kering.Roles.mailbox, stamp=helping.nowIso8601()))
        ...
        assert (
            rurls.get('registrar').get('EBErgFZoM3PBQNTpTuK9bax_U8HLJq1Re2RM1cdifaTJ').get('http') == 'http://127.0.0.1:6666'
        )
...
def da_cred():
    ...
    _, attrs = scheming.Saider.saidify(sad=a_sad, code=coring.MtrDex.Blake3_256, label=scheming.Saids.d)
    ...
    _, rules = scheming.Saider.saidify(sad=r_sad, code=coring.MtrDex.Blake3_256, label=scheming.Saids.d)
...
def setup_cred(hab, registry, verifier: verifying.Verifier, seqner):
    ...
    try:
        ...
        verifier.processCredential(
            creder=creder, prefixer=prefixer, seqner=seqner, saider=coring.Saider(qb64=hab.kever.serder.said)
        )
